...
$(document).ready(function () {
  ...
  playlistData.forEach((playlist) => {
    var palylistCard = $("<div>")
      .addClass("playlist-card ")
      .attr("id", "vid-" + playlist.id);
    var playlistImg = $("<img>")
      .attr("src", playlist.thumbnail)
      .addClass("thumbnail");
    var playlistTitle = $("<h3>")
      .addClass("video-card-title")
      .html(playlist.title);
    palylistCard.append(playlistImg).append(playlistTitle);
    $("#playlist-wrapper").append(palylistCard);
  });

  $(".playlist-card").click(function () {
    $(".playlist-card").removeClass("active-card");
    $(this).addClass("active-card");
    var vidId = $(this).attr("id").split("-")[1];
    renderVideo(vidId);
  });
  function renderVideo(id) {
    ...
    videoPlaySectionData.forEach((video) => {
      if (Number(video.id) === Number(id)) {
        $("#video-player").attr(
          "src",
          "https://player.vimeo.com/video/" + video.vimeoId
        );
        $("#views-count").html(kFormatter(video.views));
